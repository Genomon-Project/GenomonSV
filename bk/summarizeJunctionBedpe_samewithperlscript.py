...
for line in hIN:
    ...
    # if F[6] == "ST-E00104:162:H03UUALXX:5:1213:13068:62242/1":
    #     print F[6]
    ...
    for key in sorted(mergedBedpe):
        ...
        # tids, tmqs1, talns1, tinseqs, tmqs2, talns2, tpinds, tcinds = mergedBedpe[key].split('\t') # for comparison
        tids, tmqs1, talns1, tinseqs, tmqs2, talns2, tpinds = mergedBedpe[key].split('\t')
        ...
        if F[0] != tchr1 or int(F[1]) > int(tend1) + checkMarginSize:
            ...
            if len(talns_a2_uniq) >= 1:
                ...
                print '\t'.join([tchr1, tstart1, tend1, tchr2, tstart2, tend2, \
                                 ...
                                 tmqs2, talns2, tpinds]) # for comparison
            ...
            if F[0] == tchr1 and F[3] == tchr2 and F[8] == tdir1 and F[9] == tdir2:
                ...
                if flag == 1:
                    ...
                    mergedBedpe[key] = '\t'.join([newIds, newMqs1, newAlns1, newInseqs, newMqs2, newAlns2, newPinds])
                    # mergedBedpe[key] = '\t'.join([newIds, newMq1, newAlns1, newInseqs, newMqs2, newAlns2, newPinds, newCinds]) # for comparison
    ...
    if match == 0:
        ...
        mergedBedpe[newKey] = F[6] + '\t' + F[7] + '\t' + '\t'.join(F[10:15]) # for comparison
